...
- `/auth/*`: passphrase login, refresh-token rotation, and logout.
...
`POST /messages/{id}/forward` currently forwards stored text messages to one or more target chats. Each accepted forward is persisted as a new outgoing message and returned in an `items` array. Media forwarding requires preserving enough WhatsApp media metadata to re-send without reading only the local file path, so it is handled separately from text forwarding.
